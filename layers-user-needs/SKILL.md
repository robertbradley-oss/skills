...
description: "Identify and prioritize user needs from available evidence."
---


# /layers-user-needs

Use the techniques relevant to the current decision. Consult `layers-intro` only if shared framework context is needed. Existing evidence can satisfy prerequisites; ask only about consequential unknowns.
...
Then work the needs the designer raises through the disciplines above, and probe for hidden ones. Choose the technique that fits — job stories by default, ODI when measurability matters, top tasks when there's a large user base. Don't run a fixed sequence.
...
These needs are the opportunities for `/layers-product-strategy`. If they're mostly assumed, consider `/layers-observed-behaviour` to gather evidence when stronger evidence is needed for the requested decision.
