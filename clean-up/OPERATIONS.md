...
This document defines the audit and controlled-removal workflows. For an authorized task-finishing request, use [TASK_FINISHING.md](TASK_FINISHING.md) to coordinate scoped code edits and file moves through normal editing tools. Such a request satisfies the separate edit-request requirement below for relevant changes; it does not authorize Apply candidates or change either removal contract. Audit scripts continue to emit review-only code and organization findings.
...
Relevant contract boundaries (summarize only when needed):
...
The current semantic declaration pass supports tracked C#, PowerShell `.ps1`, JavaScript/JSX, and TypeScript/TSX files, including modified worktree content. For C#, it finds private fields, properties, and methods whose exact identifier appears only at the declaration. For PowerShell, it finds function names that appear only at their declaration, using case-insensitive matching. For JavaScript and JSX, it finds explicit `#private` class members plus conservative unexported top-level functions, classes, and variables whose exact identifier appears only at the declaration. For TypeScript and TSX, it finds explicit private class members plus conservative unexported top-level functions, classes, interfaces, types, enums, and variables whose exact identifier appears only at the declaration. All languages use a bounded tracked source, project, markup, resource, configuration, and metadata corpus; JavaScript and TypeScript references include tracked JavaScript, JSX, TypeScript, and TSX callers. Label every finding from a modified file as changed-worktree evidence, lower its confidence, and bind its ID to both file content and Git state. The analyzer skips generated declarations, decorators or attributes, exported JavaScript and TypeScript declarations, native externs, nested C# type declarations, entry points, and common framework callback names. Comments and string literals count as references, favoring false negatives over unsafe removal advice.

Treat every result as `review`, even when the lexical signal is strong. Modified files may contain intentionally unfinished work; analyzing them improves coverage but never strengthens removal authority. Reflection, source generation, framework file conventions, dynamic imports and access, template references, external PowerShell invocation, dot-sourcing, and callers in unsupported languages are not disproven by a single-occurrence result. Report unsupported tracked source extensions as coverage gaps. Never read control files such as `GAMEPLAN.md` for tracked-code evidence.

Tracked-code review leads alone do not make an otherwise complete file and Git audit unclean. Report them alongside the `clean` verdict as separate review-only opportunities. Coverage gaps, unreadable evidence, warnings, and exhausted budgets still prevent a clean verdict.
...
- `ignored-generated` only when the exact selected path and its complete tree are ignored, no tracked or changed descendants exist, repository references do not retain it, and strong machine-verifiable build context exists. The bundled inspector recognizes conventional `bin` and `obj` directories beside a tracked `.csproj`; `node_modules` beside a tracked valid `package.json` and tracked npm, pnpm, Yarn, or Bun lockfile; and `.next` or `next-env.d.ts` when that same package proof includes a tracked Next.js dependency. Lockfiles may live at the package root or a tracked ancestor for monorepos;
...
- Accept ignored content only for a current `ignored-generated` candidate whose complete ignored tree, tracked .NET project or Node package-and-lockfile context, zero-reference result, clean descendant state, and exact fingerprint still match the approved ID.
