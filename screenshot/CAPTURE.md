...
- If you see "screen capture checks are blocked in the sandbox", "could not create image from display", or Swift `ModuleCache` permission errors in a sandboxed run, use escalation only if the current tool and permission policy allow it; otherwise report the limitation or use a supported alternative.
...
- If saving to the OS default location fails with permission errors in a sandbox, use escalation only if the current tool and permission policy allow it; otherwise report the limitation or use a supported alternative.
